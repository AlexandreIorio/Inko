...
public class Inko implements Callable {
    private static int _cptCmdText = 0;
    ...
    @CommandLine.Option(names = {"-p", "--imagePath"}, description = "Image source path")
    ...
    @CommandLine.Option(names = {"-of", "--outputformat"}, description = "Output format : jpeg, png, gif", defaultValue = "jpeg")
    ...
    private void addText(ArrayList<String> params) {
        ...
        _exifHandler.AddText(params.get(_cptCmdText));
        _cptCmdText++;
    }
    ...
    @CommandLine.Option(names = {"-d", "--date"}, description = "Get date from EXIF data ")
    void addDate(boolean called) {
        if (_exifHandler == null) throw new NullPointerException("no image path has been given");
        _exifHandler.AddExifData(ExifHandler.EXIF.DateOriginal);
    }

    @CommandLine.Option(names = {"-cm", "--cam_model"}, description = "Get model of camera from EXIF data")
    void addCamModel(boolean called) {
        if (_imgHandler == null) throw new NullPointerException("no image path has been given");
        _exifHandler.AddExifData(ExifHandler.EXIF.CameraModel);
    }

    @CommandLine.Option(names = {"-gps", "--gps_location"}, description = "Get gps location from EXIF data")
    ...
    @CommandLine.Option(names = {"-fw", "--fontwidth"}, description = "font width : bold, italic, plain")
    private void setFontWidth(String param) {
        _overlayer.SetFontWidth(param);
    }

    @CommandLine.Option(names = {"-fs", "--fontSize"}, description = "font size px")
    ...
    @CommandLine.Option(names = {"-ma", "--margin"}, description = "Margin of overlaid image/text")
